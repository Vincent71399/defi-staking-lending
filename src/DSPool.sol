...
contract DSPool is ReentrancyGuard {
        ...
        returns (uint256 totalUserCollateralValueInDsc)
    {
        ...
        for (uint256 i = 0; i < collateralTokens.length; i++) {
            ...
            if (amount > 0) {
                ...
                uint256 valueInUsd = _calculateCollateralValueFromAmount(token, amount);
                totalUserCollateralValueInDsc += valueInUsd;
            }
        }
    }

    function _calculateCollateralValueFromAmount(address token, uint256 amount)
        private
        view
        returns (uint256 valueInUsd)
    {
        ...
        valueInUsd = (amount * adjustedPrice * (10 ** i_dsc.decimals())) / 10 ** (tokenDecimal + priceFeedDecimal);
    }
    ...
    function getCollateralValueInDscFromAmount(address token, uint256 amount) external view returns (uint256) {
        return _calculateCollateralValueFromAmount(token, amount);
    }
    ...
    function getMaxRedeemableCollateral(address user, address token)
        external
        view
        isAllowedToken(token)
        returns (uint256)
    {
        (uint256 totalDscMinted, uint256 totalCollUsd) = _getAccountInformation(user);

        // If no debt, user can redeem all of this token
        uint256 deposited = s_collateralDeposited[user][token];
        if (totalDscMinted == 0) {
            return deposited;
        }

        // Required collateral in USD (scaled to i_dsc.decimals) to keep HF >= 1:
        // HF = (collUsd * LT / LIQ_PREC) * 1e18 / debt  >=  1e18
        //  =>  (collUsd * LT / LIQ_PREC) >= debt
        // Use ceil to be safe against integer division.
        uint256 requiredUsd = _ceilDiv(totalDscMinted * LIQUIDATION_PRECISION, LIQUIDATION_THRESHOLD); // USD in i_dsc.decimals()

        // If we're already at/below requirement, nothing is redeemable safely
        if (totalCollUsd <= requiredUsd) {
            return 0;
        }

        // USD headroom we can remove without dropping HF < 1
        uint256 freeUsd = totalCollUsd - requiredUsd;

        // Convert that USD value into this token's amount
        uint256 byHealth = _calculateCollateralAmountFromUsdValue(token, freeUsd);

        // Can't redeem more than deposited
        return byHealth < deposited ? byHealth : deposited;
    }

    function _ceilDiv(uint256 x, uint256 y) internal pure returns (uint256) {
        // avoids overflow from (x + y - 1)
        return x / y + (x % y == 0 ? 0 : 1);
    }

    function _calculateCollateralAmountFromUsdValue(address token, uint256 valueInUsd)
        internal
        view
        returns (uint256)
    {
        address priceFeedAddress = s_priceFeeds[token];
        (, int256 price,,,, uint8 priceFeedDecimals) = priceFeedAddress.getPrice();
        uint256 adjustedPrice = uint256(price);

        uint256 tokenDecimals = ERC20(token).decimals();
        uint256 dscDecimals = i_dsc.decimals();

        // Invert: valueUsd = amount * price * 10^dscDecimals / 10^(tokenDec + priceDec)
        // => amount = valueUsd * 10^(tokenDec + priceDec) / (price * 10^dscDecimals)
        uint256 num = valueInUsd * 10 ** (tokenDecimals + priceFeedDecimals);
        uint256 den = adjustedPrice * 10 ** dscDecimals;
        return num / den;
    }
}
